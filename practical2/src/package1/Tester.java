...
public class Tester {
	...
	public static void main(String [] args) {
		...
		//List<FibonacciHeap.HeapNode> list2=new ArrayList<FibonacciHeap.HeapNode>();
		...
		while (iter1.hasNext()) {
			fibonacciHeap.insert(iter1.next());
		}
		fibonacciHeap.consolidating();
		HeapPrinter.print(fibonacciHeap,false);
		
		printArr(FibonacciHeap.kMin(fibonacciHeap, 20));
		...
		/*
		for(int i=1;i<n;i++) {
			list2.add(fibonacciHeap.insert(i));
		}
		Collections.shuffle(list2);
		
		Iterator<FibonacciHeap.HeapNode> iter = list2.iterator();
		while (iter.hasNext()) {
			fibonacciHeap.delete(iter.next());
		}
		*/
	}
	...
	public static void printArr(FibonacciHeap.HeapNode [] arr) {
		for(FibonacciHeap.HeapNode a:arr) {
			if(a!=null) System.out.print(a.getKey()+", ");
	...
	public class HeapPrinter {
	    ...
	    static void printIndentVerbose(FibonacciHeap.HeapNode heapNode, ArrayList<Boolean> hasNexts) {
	        ...
	        List<String> content =  Arrays.asList(
	            String.format(" Rank: %d ", heapNode.getOrder()),
